...
func main() {
	...
	//元数据服务
	listener2,e:= metastore.NewListener(c2.MetaStore.MetaAddr,c2.MetaStore.Timeout,c2.MetaStore.NodePrefix)
	if e != nil {
		log.Fatal(c2.NodeAddr.TcpAddr+" init discovery failed !",e)
	}
	//初始化注册元数据
	metaStore2:= metastore.NewMetaStore(c2.DataDir,c2.NodeAddr.TcpAddr)
	register2:= metastore.NewRegister(c2.MetaStore.NodePrefix,metaStore2.NodeAddr,
		c2.MetaStore.MetaAddr,c2.MetaStore.Timeout,c2.MetaStore.HeartBeat)
	if metaStore2.MetaData !=nil {
		for db,tbMap:=range metaStore2.MetaData {
			if tbMap !=nil {
				for tb,_:=range tbMap {
					e:=register2.PutNode(db,tb)
					if e !=nil {
